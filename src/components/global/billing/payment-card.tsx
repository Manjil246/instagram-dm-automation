    ...
    >
      <div
        className={cn(
          ...
          "flex flex-col rounded-xl py-5 pl-5 pr-10 bg-background-90 h-full"
        )}
      >
        {landing ? (
        ...
        {label === "PRO" ? (
          ...
          <p className="fonnt-bold text-text-secondary">Standard</p>
        )}
        {label === "PRO" ? (
          <p className="mb-2">
        ...
        {PLANS[label === "PRO" ? 1 : 0].features.map((feature, index) => {
          return (
            <p key={index} className="mt-2 text-muted-foreground flex gap-2">
